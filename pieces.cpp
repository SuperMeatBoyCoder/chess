...
std::vector<ChessMove> King::PossibleMovement(Board* board) {
...
bool King::IsChecking(Board* board, Square king_square) {
    return abs(m_square.v - king_square.v) <= 1 && abs(m_square.h - king_square.h) <= 1;
}

std::vector<ChessMove> Pawn::PossibleMovement(Board* board) {
    std::vector<ChessMove> can_move;
    int h_delta = 1;
    if (m_color == 'B')
        h_delta = -1;
...
bool Pawn::IsChecking(Board* board, Square king_square) {
    int h_delta = 1;
    if (m_color == 'B')
        h_delta = -1;
...
std::vector<ChessMove> Night::PossibleMovement(Board* board) {
    std::vector<ChessMove> can_move;
    ChessMove this_move;
    for (int v_delta : {-1, 1}) {
        for (int h_delta : {-1, 1}) {
...
bool Night::IsChecking(Board* board, Square king_square) {
...
std::vector<ChessMove> Bishop::PossibleMovement(Board* board) {
    std::vector<ChessMove> can_move;
    ChessMove this_move;
    for (int v_delta : {-1, 1}) {
        for (int h_delta : {-1, 1}) {
...
bool Bishop::IsChecking(Board* board, Square king_square) {
...
std::vector<ChessMove> Rook::PossibleMovement(Board* board) {
    std::vector<ChessMove> can_move;
    ChessMove this_move;
    for (int v_delta : {-1, 1}) {
...
bool Rook::IsChecking(Board* board, Square king_square) {
    Square to_square = m_square;
...
std::vector<ChessMove> Queen::PossibleMovement(Board* board) {
    std::vector<ChessMove> can_move;
    ChessMove this_move;
    for (int v_delta : {-1, 0, 1}) {
        for (int h_delta : {-1, 0, 1}) {
...
bool Queen::IsChecking(Board* board, Square king_square) {
    Square to_square = m_square;
    int v_delta = 0, h_delta = 0;
    if (to_square.v < king_square.v) v_delta = 1;
    else if (to_square.v > king_square.v) v_delta = -1;
