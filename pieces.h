...
public:
    ...
    std::vector<std::pair<int, int>> PossibleMovement(Board* board) override {
        ...
        // Pawns can't be limited by the border when going straight (when I will make transformation)
        ...
        for (int v_delta : {-1, 1}) {
            if (board->Isinside(to_v + v_delta, to_h) && !board->IsEmpty(to_v + v_delta, to_h) && board->GetColor(to_v + v_delta, to_h) != color) {
    ...
    virtual std::vector<std::pair<int, int>> PossibleMovement(Board* board) override {
        ...
        for (int to_v = vertical - 1; to_v <= vertical + 1; to_v++) {
            for (int to_h = horizontal - 1; to_h <= horizontal + 1; to_h++) {
                if (board->Isinside(to_v, to_h) && !board->IsEmpty(to_v, to_h) && board->GetColor(to_v, to_h) != color) {
                    can_move.push_back(std::pair(to_v, to_h));
                }
            }
        }
